...
class ActiveAdsViewModel : ViewModel() {
    ...
    fun getOwAds(filter: AdFilter) {
        viewModelScope.launch {
            try {
                _owAds.value = TestApi.retrofitService.GetAds(Type = filter.Type, CurrentUser = filter.CurrentUser, UserId = filter.UserId, ShowAppliedCount = filter.ShowAppliedCount, ShowStatus = filter.ShowStatus).toMutableList()
    ...
    fun getAppAds(filter: AdFilter) {
        viewModelScope.launch {
            try {
                _appAds.value = TestApi.retrofitService.GetAds(Type = filter.Type, CurrentUser = filter.CurrentUser, UserId = filter.UserId, ShowAppliedCount = filter.ShowAppliedCount, ShowStatus = filter.ShowStatus).toMutableList()
                reset()
            }catch (e: Exception) {
                Log.d("owadsfail",e.message!!)
            }
